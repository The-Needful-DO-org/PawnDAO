  ...
  <div class="xl:max-w-7xl drop-shadow-xl border border-black/20 w-full rounded-md justify-between items-stretch px-5 xl:px-5 py-5">
    <div class="mx-auto w-full lg:w-1/2 md:p-10 py-5 md:py-0">
      ...
      <form id="LoanRequestForm" class="w-full mt-5 sm:mt-8" onsubmit={loanRequestSubmit}>
        ...
        <!-- Custom Desired Token Buttons -->
        {#each customDesiredTokens as token}
          <input bind:group={desired_assets} class="btn" type="checkbox" name="desired_asset_canister_ids" value={token.canister_id} aria-label={token.symbol || 'Custom'} />
        {/each}
        ...
        <!-- Filter -->
              ...
              <input bind:group={desired_assets} class="btn" type="checkbox" name="desired_asset_canister_ids" value="kylwo-viaaa-aaaaq-aae7a-cai" aria-label="TENDY" />
